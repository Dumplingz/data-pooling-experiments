...
if __name__ == '__main__':
    ...
    table_query_load = f"""
            ...
            SELECT * FROM read_csv('{agent_des[0]}',
                delim = '|', header = false,
                columns = {{
            ...
            SELECT * FROM read_csv('{agent_des[1]}',
                delim = '|', header = false,
                columns = {{
                    'O_ORDERKEY': 'INTEGER NOT NULL',
                    'O_CUSTKEY': 'INTEGER NOT NULL',
...
SELECT DISTINCT o1.o_custkey FROM ORDERS1 o1 JOIN ORDERS2 o2 ON o1.o_custkey = o2.o_custkey;"""
    ...
    os.makedirs(f"experiments/distinct_join", exist_ok=True)
    ...
    for _ in range(num_trials):
        ...
        conn.execute(table_query_load)

        # res = conn.execute(join_query).fetchall()
        res = conn.execute(join_query).fetchdf()
        ...
        with open(f"experiments/distinct_join/{num_MB}.csv", "a") as file:
